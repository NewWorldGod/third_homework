# -*- coding: utf-8 -*-
"""
手搓最小LLM —— 使用CPU 训练（实验作业三参考框架）
对应教材第5 章《Transformer 模型》
单文件、零依赖外部数据：内置唐诗语料，字符级GPT，CPU 数分钟~二十分钟训完。
用法示例：
python min_llm.py                 # 基线：训练2000 步并生成
...
def build_corpus(extra_file=None):
    """拼接语料；若同目录下有corpus_extra.txt（学生自备），自动追加"""
    text = "\n".join(POEMS)
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_size):
        ...
        self.proj = nn.Linear(n_embd, n_embd)      # 输出投影W_O
...
class MiniGPT(nn.Module):
    ...
    def _init_weights(self, m):
        """GPT 标准初始化：权重N(0, 0.02)，偏置清零→初始loss ≈ln V"""
    ...
    def generate(self, idx, max_new_tokens=200, temperature=1.0, top_k=None):
        """自回归生成：每步预测下一个字符并拼接回输入（教材代码5-6 思想）"""
        self.eval()
        for _ in range(max_new_tokens):
            ...
            if top_k is not None:
                ...
                logits[logits < kth] = float("-inf")  # 只保留top-k
...
def main():
    ...
    ap.add_argument("--no_pos", action="store_true", help="去掉位置编码（对照实验5）")
    ...
    print(f"训练完成，耗时{train_time/60:.1f} 分钟| 最终loss {losses[-1]:.4f} "
          f"(前100 步均值{sum(losses[:100])/100:.4f})")

    # loss 曲线（英文标签避免中文乱码）
    if args.no_pos:
        tag = "no_pos"
    elif args.block_size != 128:
        tag = f"L{args.n_layer}_E{args.n_embd}_T{args.block_size}_lr{args.lr:g}"
    else:
    ...
    try:
        ...
        plt.title(f"Training Loss (n_layer={args.n_layer}, n_embd={args.n_embd}, "
                  f"lr={args.lr}, pos={'off' if args.no_pos else 'on'})")
        plt.tight_layout()
        plt.savefig(f"loss_curve_{tag}.png", dpi=150)
        print(f"已保存loss_curve_{tag}.png")
    except ImportError:
        print("未安装matplotlib，跳过曲线绘制")

    # 保存模型checkpoint，供采样实验（exp6）复用
    torch.save({"state_dict": model.state_dict(), "vocab_size": tok.vocab_size,
    ...
    # 生成：提示词「春」「月」各 2 段
    model.to(device)
    gen_name = "no_pos" if args.no_pos else "base"
    with open(f"generated_{gen_name}.txt", "w", encoding="utf-8") as f:
        for prompt in ["春", "月"]:
            for k in range(2):
                ...
                header = (f"生成示例（temperature={args.temperature}, top_k={args.top_k}, "
                          f"提示词「{prompt}」，第{k + 1}段）:")
                print("=" * 50)
                print(header)
                print(sample)
                f.write(header + "\n" + sample + "\n\n")
    print(f"已保存generated_{gen_name}.txt")
